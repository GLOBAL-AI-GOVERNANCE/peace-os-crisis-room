...
export function excellentGate(s,d,rubric){const b=scoreBreakdown(s,d);return b.evidence_marking>=Number(rubric.excellent_gate.minimum_evidence_marking)&&s.correct_confidence_range.includes(d.confidence)&&s.correct_corroboration_range.includes(d.corroboration)&&s.correct_authenticity_range.includes(d.authenticity)&&releaseScore(s,d.release_id)===15&&actionPlanValid(s,d.actions);}
...
export function performanceLabel(s,d,rubric){const score=totalScore(s,d,rubric);if(score>=90&&excellentGate(s,d,rubric))return 'Excellent governance discipline';if(score>=75)return 'Credible crisis handling';if(score>=60)return 'Mixed outcome';return 'Governance failure risk';}
