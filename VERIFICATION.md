...
| Deployed GitHub Pages browser validation | **PENDING LIVE EXECUTION** | The release controller must complete exact-commit asset and interactive browser journeys before publishing the prerelease. |
...
The prerelease may be published only after the exact reviewed source is integrated, required checks pass, Pages deploys the exact final commit, automated deployed-browser journeys pass, deterministic artifacts are rebuilt, provenance matches the final commit, and release assets reverify after download.
